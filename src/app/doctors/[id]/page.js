          ...
          <CardContent className="grid gap-6">
            <div className="grid sm:grid-cols-2 gap-4">
              <InfoItem
                ...
                label="Degree"
                value={doctorInfo?.degree || 'N/A'}
              />
              <InfoItem
                icon={<StethoscopeIcon />}
                label="Experience"
                value={doctorInfo?.experience || 'N/A'}
              />
              <InfoItem
                icon={<GenderMaleIcon />}
                label="Gender"
                value={doctorInfo?.gender || 'N/A'}
              />
              <InfoItem
                icon={<PlusIcon />}
                label="Hospital"
                value={doctorInfo?.hospital || 'N/A'}
              />
              <InfoItem
                icon={<ClockIcon />}
                label="Appointment Time"
                value={doctorInfo?.appointmentTime || 'N/A'}
              />
              <InfoItem
                icon={<HomeIcon />}
                label="Fees"
                value={doctorInfo?.fees ? `$${doctorInfo.fees}` : 'N/A'}
              />
              <InfoItem
                icon={<PhoneIcon />}
                label="Contact"
                value={doctorInfo?.number || 'N/A'}
              />
              <InfoItem
                icon={<MapPinIcon />}
                label="Address"
                value={doctorInfo?.address || 'N/A'}
              />
            </div>
            <div>
              <h3 className="font-semibold mb-2">Bio</h3>
              <p className="text-muted-foreground">{doctorInfo?.bio || 'No bio available'}</p>
            </div>
